...
public class GetUserOrderTests {
    ...
    @Test
    @DisplayName("Получение списка заказов авторизированного юзера")
    public void getOrdersUserTest() {
        ...
        ValidatableResponse response = orderClient.getOrdersByAuthorization(accessToken);
        actualStatusCode = response.extract().statusCode();
        Assert.assertEquals(SC_OK, actualStatusCode);
    }

    @Test
    @DisplayName("Получение списка заказов НЕавторизированного юзера")
